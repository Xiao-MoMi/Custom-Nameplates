...
public class AdvanceManagerImpl implements AdvanceManager {
    ...
    private static final Set<String> DEFAULT_UNIHEX = new HashSet<>(List.of("unifont.zip"));
    ...
    private void init() {
                        ...
                        if (entry.getName().endsWith(".hex") && !entry.isDirectory()) {
                            try (BufferedReader reader = new BufferedReader(new InputStreamReader(zipFile.getInputStream(entry)))) {
                                ...
                                for (int i = 0x3001; i <= 0x30FF; i++) {
                                    char c = (char) i;
                                    yml.set(CharacterUtils.char2Unicode(c), 8);
                                }
                                ...
                                for (int i = 0xAC00; i <= 0xD7AF; i++) {
                                    char c = (char) i;
                                    yml.set(CharacterUtils.char2Unicode(c), 7);
                                }
        ...
        this.templateConfigConsumersMap.put("bitmap", (id, section) -> {
            ...
            registerCharacterFontData(id, bitmapCache);
        });
            ...
            if (!unicodeCache.exists()) {
                ...
                try {
                    ...
                    for (int a = 0; a < 256; a++) {
                        ...
                        int characterSize = width / 16;
                        for (int i = 0; i < 16; i++) {
                            for (int j = 0; j < 16; j++) {
                                int x_upper = i * characterSize;
                                int x_lower = i * characterSize;
                                outer:
                                for (int x = i * characterSize; x < (i+1) * characterSize; x++) {
                                    for (int y = j * characterSize; y < (j+1) * characterSize; y++) {
                                        int rgb = bufferedImage.getRGB(x, y);
                                        int alpha = (rgb >> 24) & 0xff;
                                        if (alpha != 0) {
                                            x_lower = x;
                                ...
                                outer:
                                for (int x = (i+1) * characterSize - 1; x >= i * characterSize; x--) {
                                    for (int y = j * characterSize; y < (j+1) * characterSize; y++) {
                                        int rgb = bufferedImage.getRGB(x, y);
                                        int alpha = (rgb >> 24) & 0xff;
                                        if (alpha != 0) {
        ...
        this.templateConfigConsumersMap.put("ttf", (id, section) -> {
            ...
            registerCharacterFontData(id, ttfCache);
        });
    }
