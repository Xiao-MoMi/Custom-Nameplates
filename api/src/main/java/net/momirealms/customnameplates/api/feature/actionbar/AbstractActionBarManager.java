...
public abstract class AbstractActionBarManager implements ActionBarManager, JoinQuitListener {
    ...
    @Override
    public ActionBarConfig addConfig(ActionBarConfig config) {
        ActionBarConfig previous = configs.put(config.id(), config);
        resetArray();
        return previous;
    }

    @Override
    public ActionBarConfig removeConfig(String name) {
        ActionBarConfig previous = configs.remove(name);
        resetArray();
        return previous;
    }
        ...
        for (Map.Entry<String, Object> entry : document.getStringRouteMappedValues(false).entrySet()) {
            ...
            this.configs.put(entry.getKey(),
                    ActionBarConfig.builder()
                            ...
                            .carouselText(ConfigUtils.carouselTexts(section.getSection("text-display-order")))
                            .build()
            );
        }
    }
