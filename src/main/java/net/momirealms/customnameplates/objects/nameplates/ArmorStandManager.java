...
public class ArmorStandManager {
    ...
    public void addDefault() {
        Set<Map.Entry<String, Double>> entries = CustomNameplates.plugin.getNameplateManager().getTextMap().entrySet();
        entries.forEach(map -> addArmorStand(map.getKey(), new FakeArmorStand(this, owner, new TextCache(owner, map.getKey()), map.getValue())));
    }
    ...
    public void ascent() {
        for (ArmorStand a : armorStandArray) {
            a.setOffset(a.getOffset() + ChatBubblesManager.lineSpace);
            a.teleport();
        }
    }
    ...
    public void countdown(String random, ArmorStand as) {
        ...
        }.runTaskLater(CustomNameplates.plugin, ChatBubblesManager.stayTime * 20L);
    }
}