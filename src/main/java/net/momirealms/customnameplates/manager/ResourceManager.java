...
public class ResourceManager {
    ...
    public void generateResourcePack() {
        File gd_file = new File(CustomNameplates.plugin.getDataFolder() + File.separator + "ResourcePack");
        deleteDirectory(gd_file);
        File font_file = new File(CustomNameplates.plugin.getDataFolder() + File.separator + "ResourcePack" + File.separatorChar + ConfigManager.namespace + File.separatorChar + "font");
        File textures_file = new File(CustomNameplates.plugin.getDataFolder() + File.separator + "ResourcePack" + File.separatorChar + ConfigManager.namespace + File.separatorChar + "textures");
        ...
        loadNameplates(jsonArray_default, textures_file);
        loadBubbles(jsonArray_default, textures_file);
        loadBackgrounds(jsonArray_default, textures_file);
        extractShaders();
        saveOffsets();
        try (FileWriter fileWriter = new FileWriter(
                CustomNameplates.plugin.getDataFolder() +
    ...
    private void loadNameplates(JsonArray jsonArray_default, File textures_file) {
        if (!ConfigUtil.isModuleEnabled("nameplate")) return;
        File np_file = new File(CustomNameplates.plugin.getDataFolder() + File.separator + "nameplates");
        if (!np_file.exists()) {
            if (!np_file.mkdir()) return;
        ...
        for (File np_config_file : np_config_files) {
            char left = ConfigManager.start;
            char middle;
            char right;
            ConfigManager.start = (char) ((right = (char) ((middle = (char) (ConfigManager.start + '\u0001')) + '\u0001')) + '\u0001');
            try {
                ...
                for (SimpleChar simpleChar : simpleChars) {
                    ...
                    jo_np.add("file", new JsonPrimitive(ConfigManager.namespace + ":" + ConfigManager.np_folder_path.replaceAll("\\\\", "/") + simpleChar.getFile()));
                    ...
                    try {
                        FileUtils.copyFile(new File(np_config_file.getParent() + File.separator + simpleChar.getFile()), new File(textures_file.getPath() + File.separatorChar + StringUtils.replace(ConfigManager.np_folder_path, "\\", File.separator) + simpleChar.getFile()));
                    } catch (IOException e) {
    ...
    private void loadBubbles(JsonArray jsonArray_default, File textures_file) {
        if (!ConfigUtil.isModuleEnabled("bubbles")) return;
        File bb_file = new File(CustomNameplates.plugin.getDataFolder() + File.separator + "bubbles");
        if (!bb_file.exists()) {
            if (!bb_file.mkdir()) return;
            saveDefaultBubbles();
        }
        BUBBLES = new HashMap<>();
        ...
        for (File bb_config_file : bb_config_files) {
            ...
            char right;
            char tail;
            ConfigManager.start = (char) ((tail = (char)((right = (char)((middle = (char)(ConfigManager.start + '\u0001')) + '\u0001')) + '\u0001')) + '\u0001');
            try {
                ...
                for (SimpleChar simpleChar : simpleChars) {
                    ...
                    jo_bb.add("file", new JsonPrimitive(ConfigManager.namespace + ":" + ConfigManager.bb_folder_path.replaceAll("\\\\","/") + simpleChar.getFile()));
                    ...
                    try {
                        FileUtils.copyFile(new File(bb_config_file.getParent() + File.separator + simpleChar.getFile()), new File(textures_file.getPath() + File.separatorChar + StringUtils.replace(ConfigManager.bb_folder_path, "\\", File.separator) + simpleChar.getFile()));
                    }
                    catch (IOException e){
    ...
    private void loadBackgrounds(JsonArray jsonArray_default, File textures_file) {
        if (!ConfigUtil.isModuleEnabled("background")) return;
        File bg_file = new File(CustomNameplates.plugin.getDataFolder() + File.separator + "backgrounds");
        if (!bg_file.exists()) {
            if (!bg_file.mkdir()) {
        ...
        for (File bg_config_file : bg_config_files) {
            try {
                ...
                char oStart = ConfigManager.start;
                char o1 = (char) (oStart + '\u0001');
                char o2 = (char) (o1 + '\u0001');
                char o4 = (char) (o2 + '\u0001');
                char o8 = (char) (o4 + '\u0001');
                ...
                for (SimpleChar simpleChar : simpleChars) {
                    ...
                    jo_bg.add("file", new JsonPrimitive(ConfigManager.namespace + ":" + ConfigManager.bg_folder_path.replaceAll("\\\\","/") + simpleChar.getFile()));
                    ...
                    try {
                        FileUtils.copyFile(new File(bg_config_file.getParent() + File.separator + simpleChar.getFile()), new File(textures_file.getPath() + File.separatorChar + StringUtils.replace(ConfigManager.bg_folder_path, "\\", File.separator) + simpleChar.getFile()));
                    }
                    catch (IOException e){
    ...
    private void extractShaders() {
        if (ConfigManager.extract) {
            ...
            CustomNameplates.plugin.saveResource(path + "rendertype_text.fsh", true);
            CustomNameplates.plugin.saveResource(path + "rendertype_text.json", true);
            CustomNameplates.plugin.saveResource(path + "rendertype_text.vsh", true);
            CustomNameplates.plugin.saveResource(path + "rendertype_text_see_through.fsh", true);
            CustomNameplates.plugin.saveResource(path + "rendertype_text_see_through.json", true);
            CustomNameplates.plugin.saveResource(path + "rendertype_text_see_through.vsh", true);
        }
    }

    private void saveOffsets() {
        if (ConfigManager.offsets.size() != 0){
            for (int offset : ConfigManager.offsets) {
    ...
    private void saveDefaultNameplates() {
        ...
        for (String name : png_list) {
            for (String part : part_list) {
                CustomNameplates.plugin.saveResource("nameplates" + File.separatorChar + name + part, false);
            }
        }
    }
    ...
    private void saveDefaultBubbles() {
        ...
        for (String name : png_list) {
            for (String part : part_list) {
                CustomNameplates.plugin.saveResource("bubbles" + File.separatorChar + name + part, false);
            }
        }
    }
    ...
    private void saveDefaultBackgrounds() {
        ...
        for (String bg : bg_list) {
            CustomNameplates.plugin.saveResource("backgrounds" + File.separatorChar + bg + ".png", false);
        }
        String[] config_list = new String[]{"bedrock_1", "bedrock_2", "bedrock_3"};
        for (String config : config_list) {
            CustomNameplates.plugin.saveResource("backgrounds" + File.separatorChar + config + ".yml", false);
        }
    }
    ...
    private JsonObject getOffsetFontChar(int height, char character) {
        ...
        jsonObject.add("file", new JsonPrimitive(ConfigManager.namespace + ":" + ConfigManager.ss_folder_path.replaceAll("\\\\","/") + "space_split.png"));
    ...
    private void saveSplit(String path) {
        try{
            CustomNameplates.plugin.saveResource("space_split.png", false);
            FileUtils.copyFile(new File(CustomNameplates.plugin.getDataFolder(),"space_split.png"), new File(path + File.separator + StringUtils.replace(ConfigManager.ss_folder_path, "\\", File.separator) + "space_split.png"));
            new File(CustomNameplates.plugin.getDataFolder(),"space_split.png").delete();
        }catch (IOException e){
    ...
    private void hookCopy(File generated) {
        ...
        if (ConfigManager.oraxenHook){
            try {
                FileUtils.copyDirectory(new File(generated, File.separator + ConfigManager.namespace), new File(Bukkit.getPluginManager().getPlugin("Oraxen").getDataFolder() + File.separator + "pack"+ File.separator + "assets" + File.separator + ConfigManager.namespace));
            }
            catch (IOException e){
                e.printStackTrace();
